...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Split the original dataset (`data.csv`) into two splits: train and val, from the given ratio."
    )
    parser.add_argument('-i', '--input', type=str,
                        help="input csv file - csv_path")
    parser.add_argument('-o', '--output', type=str,
                        help="path to the directory of the output csvs (default = ‘.’)", default=".")
    parser.add_argument('-k', '--k_split', type=int,
                        help="ratio of the train subset (default = 0.8, meaning is 5)", default=5)
    parser.add_argument('-s', '--random_seed', type=int,
                        help="seed for the randomization process (default = 0)", default=0)
...
def kfold_stratified_split(parser):
    ...
    y = df['class_id']
    X = df['obj_id']
    skf = StratifiedKFold(n_splits=parser.k_split,
                          random_state=parser.random_seed, shuffle=True)
    i = 1
