...
def parse_args():
    ...
    parser.add_argument('-r','--raito',type=float,help="ratio of the train subset (default = 0.8)",default=0.8)
...
def stratified_split(parser):
    '''
    To split the original dataset (`data.csv`) into two splits: train and val, from the given ratio.
    Arguments:
    `csv_path` (str): path to the `data.csv` file
    `out_dir` (str): path to the directory of the output csvs (default = ‘.’)
    `ratio` (float): ratio of the train subset (default = 0.8)
    `random_seed` (int): seed for the randomization process (default = 0)
    Results: write two csv(s) `train.csv` and `val.csv` of the same format as `data.csv` to `out_dir`

    '''
    df = pd.read_csv(parser.input)
    y=df['class_id']
    X=df['obj_id']
    X_train, X_test, y_train, y_test = train_test_split( X, y, train_size=parser.raito, random_state=parser.random_seed)
